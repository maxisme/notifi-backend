async function getGithubReleasesJson(event) {
  ...
  if (!response) {
    ...
    if (response.status !== 200) {
      throw new Error('cannot fetch release: ' + response.statusText);
    }
...
async function getGithubRelease(event) {
  ...
  for (const asset of release['assets']) {
    if (asset['browser_download_url'].indexOf('.dmg') !== -1) {
      dmgDownloadURL = asset['browser_download_url'];
    }
  }

  const xml = `<?xml version="1.0" encoding="utf-8"?>
<rss version="2.0" xmlns:sparkle="https://notifi.it/xml-namespaces/sparkle" 
  xmlns:dc="https://notifi.it/dc/elements/1.1/">
  <channel>
    <item>
